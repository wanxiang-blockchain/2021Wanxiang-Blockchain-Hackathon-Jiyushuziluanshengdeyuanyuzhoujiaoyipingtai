...
contract BestNftExchange is Administrable, ERC1155Receiver {
    ...
    function getOrdersByExtoken(address token) external view returns (Order[] memory result) {
        ...
        IERC20 exToken = IERC20(token);
        uint[] ids = _extokenOrderIds[token];
    ...
    function getUserBids(address userAddr) external view returns (UserBidHistory[] memory) {
        return _userBidQueue[userAddr];
    }

    function getUserClosedBids(address userAddr) external view returns (UserBidHistory[] memory) {
        UserBidHistory[] ubh = getUserBids(userAddr);
        UserBidHistory[] ubhClosed;
        for (uint i=0;i<ubh.length;i++){
            if(_orders[ubh[i].orderId]) {
                continue;
            }
            ubhClosed.push(ubh[i]);
        }
        return ubhClosed;
    }
    ...
    function _bid(address bidder,uint256 orderId, uint256 price,bool isTook) internal {
        BidEntity be = BidEntity(price,bidder,isTook);
        _bidQueue[orderId].push(be);
        UserBidHistory ubh = UserBidHistory(price,orderId,isTook);
        _userBidQueue[bidder].push(ubh);
        emit OrderBid(orderId,price);
    }

    function _match(Order order,uint256 orderId,uint256 price,address receiver,bool isSetPrice) internal {

        order.nft.safeTransferFrom(address(this), receiver, order.tokenId, order.amount, "from TEAsWAP");
        uint256 exFee = SafeMath.div(SafeMath.mul(price,_feePer10000),10000);
        uint256 restCost = SafeMath.sub(price,exFee);
        if(order.exchangeToken != address(0)){
            require(order.exchangeToken.balanceOf(address(this)) > price, "NftExchange: insufficient balance");
            order.exchangeToken.safeTransferFrom(address(this), order.owner, restCost);
            order.exchangeToken.safeTransferFrom(address(this), owner(), exFee);
...
//           require(address(this).balance > price, "NftExchange: insufficient balance");
    ...
    function bid(uint256 id,uint256 price) external {
        ...
        uint256 buyerBalance = order.exchangeToken.balanceOf(msg.sender);
        require(buyerBalance >= price, "NftExchange: insufficient balance");
        require(price <= order.price, "NftExchange: exceed origin price");
        BidEntity[] Bids = _bidQueue[id];

        if (Bids && Bids.length != 0){
            BidEntity latestBid = Bids[Bids.length-1];
            require(latestBid.price < price,"NftExchange: lower price than other bids");
        }

        order.exchangeToken.safeTransferFrom(msg.sender, address(this), price);
    ...
    function withdrawAll(address take) external onlyAdmins {
        if(take!=address(0)){
            withdrawAmount(take, token.balanceOf(address(this)));
        }else{
...
// File @openzeppelin/contracts/token/ERC1155/IERC1155MetadataURI.sol@v3.2.0
...
// File @openzeppelin/contracts/token/ERC1155/ERC1155.sol@v3.2.0
