...
contract NftExchange is Administrable,ERC721Holder,ERC1155Receiver {
        ...
        if (Bids.length != 0){
            ...
            for(uint256 i = Bids.length;i>0; i--  ){
                if(Bids[i-1].isTook==false){
                    maxBidPrice = Bids[i-1].price;
                    break;
                }
            }
            require(maxBidPrice < price,"NftExchange: lower price than other bids");
        }
    ...
    function addOrderbyOwner(IERC1155Collection nft, uint256 id, uint256 amount, address payable owner, uint256 price,uint256 royalty,address exchangeToken) public  {
