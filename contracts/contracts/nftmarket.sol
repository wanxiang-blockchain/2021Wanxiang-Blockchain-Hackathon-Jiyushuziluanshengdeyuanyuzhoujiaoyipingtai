...
// File @openzeppelin/contracts/token/ERC1155/IERC1155.sol@v3.2.0
...
// File @openzeppelin/contracts/token/ERC1155/IERC1155Receiver.sol@v3.2.0
...
// File @openzeppelin/contracts/token/ERC1155/ERC1155Receiver.sol@v3.2.0
...
contract NftExchange is Administrable,ERC721Holder,ERC1155Receiver {
    ...
    mapping (uint256 => BidEntity[]) public _bidQueue;
    ...
    constructor(uint256 feePer10000) public {
        ...
        _feeOwner = 0x34731834778cFF1955A454e23D091a081EBBDFE9;
    }
    ...
    function getUserClosedBids(address userAddr) external view returns (UserBidHistory[] memory) {
        ...
        uint256 length = 0;
        for (uint256 i=0;i<ubh.length;i++){
    ...
    function getOrderHistory(uint256 orderid) external view returns (Order memory) {
    ...
    function bid(uint256 id,uint256 price) external {
        ...
        if (Bids.length != 0){
            ...
            require( !hasPrice, "NftExchange: has same bid price");
        }
    ...
    function withdrawBid (uint256 id,uint256 price) external{

        BidEntity[] memory orderbids = _bidQueue[id];
        Order memory bidOrder = _orderHistory[id];
        require(address(bidOrder.exchangeToken) != address(0), "NftExchange: can not take BNB");
        bool hasBid = false;

        for(uint256 i = 0 ;i < orderbids.length; i++){
            if(orderbids[i].bidder==msg.sender && orderbids[i].price==price && orderbids[i].isTook==false){
                IERC20(bidOrder.exchangeToken).transfer(msg.sender,price);
                _bidQueue[id][i].isTook=true;
                hasBid = true;
                break;
            }
        }

        require(hasBid,"NftExchange: no bid");

        UserBidHistory[] memory ubh = getUserBids(msg.sender);

        for (uint256 i=0;i<ubh.length;i++){
            if(ubh[i].price == price && ubh[i].orderId == id) {
                _userBidQueue[msg.sender][i].isTook = true;
                break;
            }
        }
    ...
    function withdrawBidBNB (uint256 id,uint256 price) external{

        BidEntity[] memory orderbids = _bidQueue[id];
        Order memory bidOrder = _orderHistory[id];
        require(address(bidOrder.exchangeToken) == address(0), "NftExchange: can not take ERC20");
        bool hasBid = false;
    ...
    function _adminrevoke (uint256 id) internal{
        Order memory order = _orders[id];
        require(address(order.nft) != address(0), "NftExchange: bad id");
        order.nft.safeTransferFrom(address(this), order.owner, order.tokenId, order.amount, "");
        removeOrder(id);
    }

    function revokebybyadmin(uint256 id) external onlyAdmins {
        _adminrevoke(id);
    }

    function revokeall() external onlyAdmins {

        uint256 index  = 0;
        uint256 orderId = _next[0];
        uint256 [] memory  orderids = new uint256 [] (_totalOrder);

        while (orderId > 0) {
            Order memory order = _orders[orderId];
            if(address(order.nft) != address(0)){
    ...
    function setPrice(uint256 id, uint256 price) external {
        ...
        require(address(order.nft) != address(0), "NftExchange: bad id");
        require(order.owner == msg.sender, "NftExchange: not order owner");
        ...
        if (Bids.length != 0){
            ...
            if(latestBid.price == price){
                ...
                Bids[Bids.length-1].isTook = true;

                UserBidHistory[] memory ubh = getUserBids(Bids[Bids.length-1].bidder);
    ...
    function addOrder(uint256 orderid, IERC1155Collection nft, uint256 id, uint256 amount, address payable owner, uint256 price,uint256 royalty,address exchangeToken) public onlyAdmins {

        require(orderid != 0, "NftExchange: orderid cannot be 0");
        nft.safeTransferFrom(nft.ownerOf(id), address(this), id);

        _orders[orderid] = Order(nft, id, amount, owner, price,exchangeToken,royalty);
        _orderHistory[orderid] = Order(nft, id, amount, owner, price,exchangeToken,royalty);
        _next[_tail] = orderid;
    ...
    function addOrderbyOwner(uint256 orderid,IERC1155Collection nft, uint256 id, uint256 amount, address payable owner, uint256 price,uint256 royalty,address exchangeToken) public  {

        require(msg.sender == nft.ownerOf(id), "NftExchange: no owner");
        bool hasOrderId = false;
        require(_tail != orderid, "NftExchange: dumplicate orderid");
        require(orderid != 0, "NftExchange: orderid cannot be 0");
        uint256 i = _tail;
        while(_previous[i]!=0){
            if(_previous[i] == orderid ){
                hasOrderId = true;
                break;
            }
            i = _previous[i];
        ...
        require(!hasOrderId, "NftExchange: dumplicate orderid 2");
    ...
    function batchAddOrder(uint256[] memory orderids, IERC1155Collection [] memory nfts, uint256 [] memory ids, uint256 [] memory amounts, address payable [] memory owners, uint256 [] memory prices,uint256 [] memory royalties,address [] memory exchangeTokens) public onlyAdmins {

        for(uint256 i = 0; i<nfts.length; i++){
            addOrder(orderids[i],nfts[i], ids[i], amounts[i], owners[i], prices[i],royalties[i],exchangeTokens[i]);
        }
